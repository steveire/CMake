...
//----------------------------------------------------------------------------
cmCPackAPKGenerator::cmCPackAPKGenerator()
{
...
//----------------------------------------------------------------------------
bool cmCPackAPKGenerator::CanGenerate()
{
...
int cmCPackAPKGenerator::PackageFiles()
{
  ...
  const char *barDescriptor = this->GetOption("CPACK_APK_DESCRIPTOR");
  ...
  if (barDescriptor)
    ...
    const char *extraLdLibraryPaths =
                          this->GetOption("CPACK_APK_EXTRA_LD_LIBRARY_PATHS");
    if (extraLdLibraryPaths)
      {
      std::string processed = ":";
      processed += extraLdLibraryPaths;
      std::replace(processed.begin(), processed.end(), ';', ':');
      this->SetOption("CPACK_APK_EXTRA_LD_LIBRARY_PATHS", processed.c_str());
      }

    this->ConfigureString(packageHeaderText, res);
    }
    const char *entryPoint = this->GetOption("CPACK_APK_ENTRY_POINT");

    if (!entryPoint)
      {
      cmCPackLogger(cmCPackLog::LOG_ERROR,
        "CPACK_APK_ENTRY_POINT must be set." << std::endl);
      return 0;
      }

    std::vector<std::string> binaryFiles;
    cmSystemTools::ExpandListArgument(this->GetOption("CPACK_APK_BINARIES"),
                                      binaryFiles);
    ...
    for(std::vector<std::string>::const_iterator it = this->files.begin();
      ...
      if(isEntryPoint
          && config == "RELEASE"
          && this->IsOn("CPACK_APK_CASCADES"))
        {
        res += "    <entryPointType>Qnx/Cascades</entryPointType>\n";
        }
      }

    if(const char *category = this->GetOption("CPACK_APK_CATEGORY"))
      {
      res += "\n    <category>";
      res += category;
      res += "</category>\n";
      }
    if(const char *permissions = this->GetOption("CPACK_APK_PERMISSIONS"))
      {
      res += "\n";
    ...
    if(const char *authorId = this->GetOption("CPACK_APK_AUTHOR_ID"))
      {
      res += "\n";
      if (this->GetOption("CPACK_APK_AUTHOR_ID_FROM_DEBUG_TOKEN"))
        {
        cmCPackLogger(cmCPackLog::LOG_ERROR,
      ...
      res += "</authorId>\n";
                    ...
                    = this->GetOption("CPACK_APK_AUTHOR_ID_FROM_DEBUG_TOKEN"))
      {
      command += " -devMode -debugToken ";
      command += token;
      }

    const char *chrome = this->GetOption("CPACK_APK_WINDOW_CHROME");
    const char *transparent = this->GetOption("CPACK_APK_WINDOW_TRANSPARENT");
    const char *orientation = this->GetOption("CPACK_APK_WINDOW_ORIENTATION");
    ...
    for(std::vector<std::string>::const_iterator
      ...
      {
      if (cmHasLiteralPrefix(*it, "CPACK_APK_ENV_VAR_"))
        {
        std::string var = it->c_str() + sizeof("CPACK_APK_ENV_VAR_") - 1;
        res += "    <env var=\"" + var + "\" "
